    ...
    return new Promise((resolve, reject) => {
        ...
        fileStream.on('open', () => {
            axios.get(url, { responseType: 'stream' })
                ...
                .catch(async err => {
                    console.log(`Error downloading segment ${url}\n\tMessage: ${err.message}\n\tRetrying...`)
                    await sleep(500);
                    axios.get(url, { responseType: 'stream' })
                        .then(response => {
                            response.data.pipe(fileStream);
                        ...
                        .catch(async err => {
                            reject({
                                message: `Error downloading segment ${url}\n\tMessage: ${err.message}`
                            });
                        });
                });
        })
            ...
            if (file.indexOf('init') > -1) {
                ...
                try {
                    ...
                    try {
                        ...
                        reject({
                            message: `Unable to append segment.\n\tUrl: ${url}\n\tFile: ${file}\n\tError: ${err.message}`
                        });
                    }
                }
            }
        ...
        fileStream.on('error', (err) => {
            reject({
                message: `Error writing file segment.\n\tMessage: ${err.message}`
            });
        });
    });
}
...
const downloadDASHVideoStream = async (stream, emitter, startSegment = 0) => {
    ...
    try {
        ...
        console.log(`Error downloading segment list ${uri}\n\tError: ${e.message}`);
        return null;
    }
